...
// ---------------------------------------------------------------------------
// Data (centres, retainers, mockUsedHours, nonMediaClients) now arrives as
...
export default function ClientOpsDashboard({ centres, retainers, mockUsedHours, nonMediaClients }) {
  ...
  const filtered = rows.filter((r) => {
    const matchesLead = leadFilter === "All" || r.lead === leadFilter;
  ...
  const hoursAtRiskCount = rows.filter((r) => {
    ...
    const usedObj = mockUsedHours[r.client];
      ...
      <div className="max-w-[1400px] mx-auto px-6 py-8">
        ...
        <div className="flex items-start justify-between flex-wrap gap-4 mb-6 pb-6 border-b border-[#E4DFD1]">
          <div>
            ...
            <p className="text-sm text-[#5B5648] mt-1">Media spend pacing, KPI tracking and retainer hours — one screen, no Teamwork tab-switching.</p>
          </div>
        ...
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          ...
          <div className="flex gap-1">
            {["All", "Miya", "Other"].map((l) => (
              <button
                key={l}
                onClick={() => setLeadFilter(l)}
                className="text-xs px-3 py-2 rounded border"
                style={{
                  background: leadFilter === l ? "#242019" : "white",
                  color: leadFilter === l ? "white" : "#5B5648",
                  borderColor: leadFilter === l ? "#242019" : "#E4DFD1",
                }}
              >
                {l}
              </button>
                ...
                <tr key={r.client} className="border-t border-[#EFEBDE] hover:bg-[#FBFAF6]">
                  ...
                  <td className="px-4 py-3">
                    <HoursBar
                      ...
                      usedObj={mockUsedHours[r.client] || { bau: 0, tactical: 0 }}
        ...
        <div className="mt-6 flex items-start gap-2 text-xs text-[#8A8578] max-w-3xl">
          ...
          <p>
            Media spend and KPI figures are real, pulled from the March–August 2026 tabs of the OI Master Report, scoped to Miya's former book.
            KPI targets are the average of the 5 months <em>before</em> the current one (Mar–Jul), so August's actuals are measured against a real historical baseline, not a number that includes itself — confirm if a longer window (e.g. 12 months) is preferred once more history is loaded in. Workington only has 3 months of data so its target is less reliable.
            ...
            Hours <em>used</em> are placeholder values standing in for the Teamwork API. Annual retainer pacing assumes an April–March UK tax year, per your confirmation.
          </p>
        </div>
      </div>
    </div>
